...
class Transaction(models.Model):
    ...
    grand_total = models.DecimalField(max_digits=10, decimal_places=2, default=0.00) # Replaces old total_amount

    # --- NEW: Payment Method ---
    payment_method = models.CharField(max_length=10, choices=PAYMENT_CHOICES, default='CASH')

    def save(self, *args, **kwargs):
        # Auto-generate a unique bill number before saving
        if not self.bill_number:
            last_transaction = Transaction.objects.all().order_by('id').last()
            if not last_transaction:
                new_id = 1
            else:
                new_id = last_transaction.id + 1
