...
from .serializers import (
    ...
    TransactionListSerializer, # Import the new serializers
...
class DashboardMetricsView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        total_revenue = transactions.aggregate(total=Sum('grand_total'))['total'] or 0
        total_sales = transactions.count()
        top_selling_items = Sale.objects.filter(transaction__in=transactions).values('jewelry_item__name').annotate(count=Count('id')).order_by('-count')[:5]
        top_selling_categories = Sale.objects.filter(transaction__in=transactions).values('jewelry_item__category').annotate(count=Count('id')).order_by('-count')[:5]
...
class InventoryReportView(APIView):
    """Provides a report of current in-stock items."""
    def get(self, request, *args, **kwargs):
        # Get counts of items currently in stock, grouped by category and name
        stock_report = JewelryItem.objects.filter(stock_status='IN_STOCK').values('category', 'name', 'style', 'material').annotate(count=Count('id')).order_by('category', 'name')
...
class JewelryItemDetailView(generics.RetrieveAPIView):
    """Retrieves the details of a single jewelry item by its ID."""
    queryset = JewelryItem.objects.all()
    serializer_class = JewelryItemSerializer

# This view is for barcode
def generate_barcode_view(request, item_id):
    """Generates a single barcode image."""
...
class BulkBarcodeExportView(APIView):
    """Accepts a list of item IDs and returns a zip file of barcode images."""
    def post(self, request, *args, **kwargs):
...
def generate_upi_qr_code(request):
    """Generates a UPI QR code image."""
    upi_id = request.GET.get('upi_id')
    amount = request.GET.get('amount')
    bill_number = request.GET.get('bill_number')

    if not all([upi_id, amount, bill_number]):
        return HttpResponse("Missing UPI details", status=400)

    # Construct the UPI payment string
    upi_string = f"upi://pay?pa={upi_id}&pn=Kavs%20Glamstone&am={amount}&tn=Bill%20No:%20{bill_number}"
...
# --- NEW: VIEWS FOR THE RECEIPTS PAGE ---

class TransactionListView(generics.ListAPIView):
    """Lists all past transactions for the receipts page, with searching."""
    queryset = Transaction.objects.all().select_related('customer').order_by('-transaction_date')
    serializer_class = TransactionListSerializer
    filter_backends = [filters.SearchFilter]
    search_fields = ['bill_number', 'customer__name', 'customer__phone_number']

class TransactionDetailView(generics.RetrieveAPIView):
    """Retrieves the full details of a single transaction for a receipt."""
    queryset = Transaction.objects.all()
